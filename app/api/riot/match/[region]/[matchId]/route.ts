/**
 * Riot Match API - Get match details by match ID
 */
...
export async function GET(
  ...
  try {
    ...
    console.log('Request parameters:', { region, matchId, timeline });
    ...
    try {
      ...
      // Determine the endpoint based on whether timeline is requested
      const endpoint = timeline ? 'timeline' : '';
      const url = `https://${routingValue}.api.riotgames.com/lol/match/v5/matches/${matchId}${endpoint ? '/' + endpoint : ''}`;
